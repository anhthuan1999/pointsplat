...
def make_grid(imgs, nrows=3, ncols=3):
...
def evaluation(model, test_loader, output_dir, output_gt, compare_with_pseudo,
               compare_with_input=False, save_as_single=False,
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if remain_scenes != test_batch[0]['remain_scenes']:
                ...
                test_batch_gs = [test_batch[0]['gs_params']]
            ...
            for iii, (out_gs, in_gs, cameras, gt_imgs, scene_idx) in enumerate(
                ...
                if evaluate_input:
                    ...
                    pred_imgs, _ = gs_utils.rasterize_gaussians_to_multiimgs(out_gs, cameras, gt_imgs)
    ...
    torch.distributed.reduce(num_images, dst=0)
    torch.distributed.reduce(num_scenes, dst=0)
    for key in metrics:
        torch.distributed.reduce(metrics[key], dst=0)
        if dist.get_rank() == 0:
...
def training(
    ...
    for step in tqdm(range(resume_from_step * accumulate_step, total_steps * accumulate_step), disable=dist.get_rank() != 0):
        ...
        if step_consider_accum == 0:
            print('Num Gaussians (train):', batch[0]['gs_params']['means'].shape[0])
        ...
        if step_consider_accum < pretrain_steps:
            ...
            for out_gs, cameras, images, in_gs in zip(out_batch_gs, batch_cameras, batch_images, batch_gs):
                pred_imgs, _ = gs_utils.rasterize_gaussians_to_multiimgs(out_gs, cameras, images)
                for pred_img, gt_img in zip(pred_imgs, images):
                    ...
                    metric_dict['train_psnr'] += psnr(pred_img.unsqueeze(0), gt_img.unsqueeze(0)).mean()
                    num_images += 1
        ...
        if (step % accumulate_step == 0) and (step_consider_accum % save_interval == 0 or step_consider_accum == total_steps - 1):
            ...
            for test_dataset, test_loader in build_testloader().items():
                metrics, metrics_input = evaluation(
                    ...
                    output_dir=output_dir + f'/eval/{test_dataset}/{step_consider_accum}',
                    output_gt=(step_consider_accum == 0),
                    compare_with_pseudo=step_consider_accum < pretrain_steps,
...
def main(argv):
    ...
    if FLAGS.only_eval == False:
        ...
        if rank == 0:
            wandb_run = wandb.init(project='3dgs_multiple-scenes', dir=FLAGS.wandb_dir)
